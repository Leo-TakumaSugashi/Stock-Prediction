...
from subFunctions import get_name, get_ListInfoFile, ListInfoFile
import os,sys
...
def main():
    ...
    Logs = "logs-keep-model/TB_logs-keep-model-"+Today
    CSV = pd.read_csv(ListInfoFile)
    for symbol in list(CSV["Symbol"]):
        print(f"  Symbol : {symbol}")
        try:
            InfoDic = get_name(symbol)
            Name = InfoDic["Name"]
            main_Process(symbol, load_dir, Logs, Name, window_size)
...
def main_Process(symbol, load_dir,Logs,Name,window_size):
    fname = os.path.join(os.getcwd(), load_dir, symbol + '_historical_data.csv')
    model_name = os.path.join(os.getcwd(), 'KEEPMODEL_checkpoint.pth')
